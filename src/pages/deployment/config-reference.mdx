...
In the following properties, `<progID>` is a placeholder for the name of the batch program - this must be a valid YAML term (lower case, alphanumerical plus hyphens, must begin with a letter), but does not need to match the Batch process class.

| Parameter | Description | Default |
| --------- | ----------- | ------- |
| `batch.backoffLimit` | Number of retries for failed batch executions | `1` |
| `batch.javaOptions` | Default Java Heap options for all programs | `-Xms1024m -Xmx1024m` |
| `batch.maxMemory` | The maximum Java heap for the batch pods | `1024m` |
...
| `batch.programs.<progID>.username` | Cúram username for the execution of the batch process (`-Dbatch.username=`) | `''` |
...
| Parameter | Description | Default |
| --------- | ----------- | ------- |
| ![Container only](https://img.shields.io/badge/-Container_only-red) `global.mq.version` | Version of [IBM MQ](https://hub.docker.com/r/ibmcom/mq) to deploy | `9.4.3.0-r2` |
...
| `xmlserver.jvm.maxMemory` | Maximum Java heap for the XML server | `768m` |
| `xmlserver.jvm.threadStackSize` | Java thread stack size for the XML server | `-Xss4m` |
| `xmlserver.jvm.jvmOptions` | Options to be passed to the JVM, e.g., minimum heap size `-Xms1024m` | `''` |
| `xmlserver.config.threadPoolSize` | XML server thread pool size | `5` |
| `xmlserver.config.threadPoolQueueSize` | XML server thread pool queue size | `200` |
| `xmlserver.config.socketTimeout` | XML server socket timeout (milliseconds) | `60000` |
...
| ![Cúram 8.1.0.0](https://img.shields.io/badge/-Cúram_8.1.0.0-green) `xmlserver.readinessProbe.initialDelaySeconds` | Seconds after container startup to initiate readiness probe | `15` |
| ![Cúram 8.1.0.0](https://img.shields.io/badge/-Cúram_8.1.0.0-green) `xmlserver.readinessProbe.periodSeconds` | Seconds between probe invocations | `60` |
...
| ![Cúram 8.1.0.0](https://img.shields.io/badge/-Cúram_8.1.0.0-green) `xmlserver.livenessProbe.initialDelaySeconds` | Seconds after container startup to initiate liveness probe | `60` |
| ![Cúram 8.1.0.0](https://img.shields.io/badge/-Cúram_8.1.0.0-green) `xmlserver.livenessProbe.periodSeconds` | Seconds between liveness probe invocations | `120` |
| ![Cúram 8.1.0.0](https://img.shields.io/badge/-Cúram_8.1.0.0-green) `xmlserver.livenessProbe.timeoutSeconds` | Number of seconds before the probe times out | `30` |
...
In Kubernetes, XML server replicas are abstracted to the single `xmlserver.serverPort` by a Kubernetes service that handles load balancing between multiple replicas.
...
| Parameter | Description | Default |
| --------- | ----------- | ------- |
| `uawebapp.imageConfig.library` | Namespace in the image registry containing your Universal Access image | `''` |
| `uawebapp.imageConfig.name` | Name of your Universal Access image to be used | `''` |
| `uawebapp.imageConfig.tag` | Image tag of your Universal Access image to be used | `latest` |
| `uawebapp.ingressPath` | Static content Ingress/Route path without a trailing forward slash (must match the [PUBLIC_URL](https://curam-spm-devops.github.io/wh-support-docs/pdf/CE/UniversalAccess_6_1_3.pdf) variable used when the application was built) | `/universal` |
| `uawebapp.readinessProbe.path` | Path for pod readiness probe (should match `ingressPath` but with trailing slash) | `/universal/` |
