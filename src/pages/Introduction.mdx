...
For enterprise-scale environments, the runbook outlines best practices for deploying on Red Hat OpenShift, Azure Kubernetes Service (AKS) and Amazon Elastic Kubernetes Service (EKS). As part of Cúram 8.2, AKS is fully supported for production use. From Cúram 8.2.1.0, EKS is supported for production, development and test environments with Java 21 only, giving organizations more flexibility in aligning with their cloud infrastructure of choice.
